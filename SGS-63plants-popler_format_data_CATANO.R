...
# Here is some template or example code that may be helpful for putting datasets 
# into the format for the LTER metacommunities working group synthesis project and 
# performing a few data checks. 

# Make one script for each dataset that converts the data from the form we originally 
# got it in to the one we need. This might involve aggregating subplots and/or 
# multiple sampling occasins by year, subsetting out sites or species, etc. It may 
# also be necessary to fill in an abundance of zero for any taxa that appear in the 
# dataset but were not observed at a particular site-year.

# Save the resulting file in the directory:
# "~/Google Drive/LTER Metacommunities/LTER-DATA/L3-aggregated-by-year-and-space". 
# This file should be ready for preliminary analalysis.

# The next step in the pipeline after this one is to run each dataset through the 
# script 3_explore_comm_dat.R. This script plots the sampling effort (to be sure 
# overvations are balanced), the species accumulation curve, and a time series of 
# species richness at each site as well as in aggragate. If these preliminiary 
# visualizations turn up something problematic, go back to the script specific to 
# the dataset and add in annotated code to fix the problem.
...
dat <- df2[ , c("YEAR", "WEB", "KIND", "TRANS", "PT", "rep", "PCT.COVER", "new_spe")]
dat <- dat %>%
...
length(unique(comm.long4$SITE_ID)) #180 0.1m2 plots, 8 years
